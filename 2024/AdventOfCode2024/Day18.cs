...
public class Day18(string inputFilePath)
{
    ...
    private static bool IsDesignPossible(IReadOnlyCollection<string> patterns, string design)
    {
        ...
        var previousWorkingCopy = workingCopy;
        for (;;)
        {
            foreach (var pattern in patterns)
            {
                if (workingCopy.StartsWith(pattern))
                {
                    workingCopy = workingCopy.Remove(0, pattern.Length);
    ...
    // returns the pattern it starts with
    private string? StartsWithPattern(IReadOnlyCollection<string> patterns, string workingCopy)
    {
        foreach (var pattern in patterns)
        {
            if (workingCopy.StartsWith(pattern))
